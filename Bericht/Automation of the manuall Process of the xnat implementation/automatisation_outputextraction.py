...
# The following code is moved into main() after all_files is defined.
...
def launch_container_with_all_files(xnat_host, project_id, command_id, wrapper_name,
    ...
    input_file_names = [f["Name"] for f in files]
...
#---------------------------------Extraction of teh output from the Conatiner--------------------------

def get_container_output(container_name):
    try:
...
print(get_container_output("mein_container"))

#-------------------------------------------------------------------------------------------------------

# The following imports are already present at the top of the file and do not need to be repeated here.
...
def main():
    ...
    # Command-JSON erzeugen und hochladen
    ...
    # Alle Dateien im Projekt finden
    ...
    for file in all_files:
        level = file["Ebene"]
        grouped_by_level[level].append(file)

    # Beispiel: alle Project-Dateien anzeigen
    for file in grouped_by_level["project"]:
        print(file["Name"], "→", file["URI"])

    # Gruppieren nach SubjectID
    grouped_by_subject = defaultdict(list)
    for file in all_files:
        if "SubjectID" in file:
            grouped_by_subject[file["SubjectID"]].append(file)

    for subject_id, files in grouped_by_subject.items():
        print(f"Files for Subject {subject_id}:")
        for f in files:
            print("  ", f["Name"])

    # Gruppieren nach SessionID
    grouped_by_session = defaultdict(list)
    for file in all_files:
        if "SessionID" in file:
            grouped_by_session[file["SessionID"]].append(file)

    for sess_id, files in grouped_by_session.items():
        print(f"Session {sess_id}:")
        for f in files:
            print("  ", f["Ebene"], f["Name"])

    # Nur einen Container für die erste Datei starten
