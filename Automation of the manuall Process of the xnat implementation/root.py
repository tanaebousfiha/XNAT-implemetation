...
#---------------------Bulklaunch----------------------------------------
def launch_container_rest(xnat_host, project_id, command_id, wrapper_name, session_id, xnat_user, xnat_password):
...
def main():
    ...
    # Wrapper aktivieren (außerhalb des except-Blocks!)
    ...
    launch_container_rest(xnat_host, project_id, command_id, wrapper_name, session_id, xnat_user, xnat_password)
...
'''Nachdem im Terminal angezeigt wurde, dass der Container erfolgreich gestartet wurde, habe ich direkt in der XNAT-Weboberfläche nachgeschaut – aber leider keinen Workflow-Eintrag, keinen Status und keinen gestarteten Container gefunden.

Ich habe anschließend in Hawki nachgefragt, aber das Problem blieb weiterhin unklar.

Dann bin ich auf eine Unterhaltung in der XNAT Discussion Group gestoßen. Dort wurde eine Teil-Erklärung gegeben, woran das Problem liegen könnte – es ging um die korrekte Verwendung der Bulklaunch-Endpunkte.

Daraufhin habe ich im Swagger UI von XNAT nachgesehen und den richtigen REST-Endpunkt gefunden. Ab diesem Moment habe ich meine Anfrage so angepasst:

python
Kopieren
Bearbeiten
response = requests.post(...)
Damit konnte ich den Container schließlich erfolgreich über die REST-API starten.'''