...
#Mehrfachauswahlen sammeln und dabei Duplikate automatisch ausfiltern
    for context in mod_data["contexts"]:
        ...
        "input-files": [
            {
                "name": "input_files",
                "description": "All files from project",
                "file-types": ["*"],
                "multiple": True
            }
        ],
...
# Dummy-Funktion – ersetzt diese durch echte Logik für XNAT-Dateiabfrage
def get_all_files_all_levels(xnat_host, project_id, xnat_user, xnat_password):
    return [{"Name": "example_file.txt", "Ebene": "project"}]
...
def main():
    ...
    for file in all_files:
        ebene = file.get("Ebene", "unknown")
        grouped_by_level.setdefault(ebene, []).append(file)

    # Reihenfolge definieren
    ebenen_reihenfolge = ["project", "subject", "xnat:mrSessionData", "xnat:petSessionData", "xnat:ctSessionData", "scan"]
    zeit_pause = 10  # Sekunden zwischen Gruppen

    # Gruppenbasiert Container starten
    for ebene in ebenen_reihenfolge:
        gruppen_dateien = grouped_by_level.get(ebene, [])
        if not gruppen_dateien:
            continue
        print(f"\nVerarbeite Ebene: {ebene} ({len(gruppen_dateien)} Dateien)")
        ...
        launch_container_with_all_files(
            ...
            files=gruppen_dateien
        ) 

        print(f"Warte {zeit_pause} Sekunden vor nächster Gruppe...")
        time.sleep(zeit_pause)
