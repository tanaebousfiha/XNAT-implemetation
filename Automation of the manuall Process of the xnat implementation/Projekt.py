...
#xAPI (Experience API/Tin Can API): Standard im E-Learning zur Protokollierung von Lernerfahrungen.
#-----------------------------------6)Command-Liste------------------------------------------------------------------------------------------------------------
...
def get_command_id_by_name(xnat_host, xnat_user, xnat_password, command_name):
    ...
    #Prüft ob das Dictionnary eine nSchlüssel Commands enthaält
    #2Bedingungen Wahr>>Führt er den Code
...
def get_all_files_all_levels(xnat_host, project_id, xnat_user, xnat_password):
    ...
    for res in resp.json().get("ResultSet", {}).get("Result", []):
        res_name = res['label']#Name dieser Ressource
        file_url = f"{xnat_host}/data/projects/{project_id}/resources/{res_name}/files?format=json"#Für jede Ressource wird die spezifische URL für die darin gespeicherten Dateien generiert
...
def launch_container_with_all_files(xnat_host, project_id, command_id, wrapper_name,
                                    xnat_user, xnat_password, all_files):
    """
    Startet einen Container mit ALLEN gesammelten Dateien im Projekt 
    """
    ...
    input_files_str = " ".join(input_file_names)#Eingabeformat für den Container-Startbefehl'
