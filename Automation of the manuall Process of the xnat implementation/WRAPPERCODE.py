...
def enable_wrapper(xnat_host, project_id, command_id, wrapper_id, xnat_user, xnat_password):
    enable_url = f"{xnat_host.rstrip('/')}/xapi/projects/{project_id}/commands/{command_id}/wrappers/{wrapper_id}/enable"# url zusammenbauen 
    print(f"Enable-URL: {enable_url}") # Debug-Ausgabe der URL
    resp = requests.post(enable_url, auth=(xnat_user, xnat_password), verify=False)# # POST-Anfrage 
    if resp.status_code == 200:
        print("Wrapper successfully enabled.")
    else:
        print(f"Failed to enable wrapper: {resp.status_code} - {resp.text}")

#---------------------Container in XNAT starten----------------------------------------

def run_container_in_xnat(xnat_host, command_id, wrapper_id, session_id, xnat_user, xnat_password):
    url = f"{xnat_host}/xapi/container-launcher/launch"
    headers = {"Content-Type": "application/json"}# Header für die Anfrage 
    ...
    print("Launching container with payload:", json.dumps(payload, indent=2))  # Debug!
...
def main():
    ...
    # --- Wrapper anlegen oder holen (fehlerrobust!) ---
    wrapper_id = None
    try:
    ...
    # --- Wrapper aktivieren ---
    enable_wrapper(xnat_host, project_id, command_id, wrapper_id, xnat_user, xnat_password)


    # --- Container starten ---
    run_container_in_xnat(xnat_host, command_id, wrapper_id, session_id, xnat_user, xnat_password)
...
if __name__ == "__main__":
    ...
    #mit HAWKKI NACHGEARBEITET https://hawki.hawk.de/chat/s4u9hdbhmhqdvujp