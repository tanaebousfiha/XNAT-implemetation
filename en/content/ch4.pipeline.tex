...
In this work, the pipeline approach was investigated using an OSA (Obstructive Sleep Apnea) prediction script as a use case.
\normalsize
...
\begin{figure}[H]
    ...
    \caption{ Diagram: Conceptual diagram of the XNAT Pipeline workflow.}
    \label{fig:pipeline}
\end{figure}
...
The POST /pipelines/launch/pipelineName endpoint~\cite{pipelinerestapi} in XNAT's REST API
 enables automated execution of registered pipelines. However, the lack of endpoints for global pipeline enablement and project assignment means that deployment and project-level setup requires manual configuration.
...
\begin{table}[H]
  \centering
  \caption{ User Guide of the pipeline method.}
  \label{tab:pipeline}
