...
\begin{figure}[H]
  \centering
  \includegraphics[width=1.1\textwidth]{en/content/mockup0.png}
  \caption{GUI Mockup 1 for Docker Container Automation in XNAT}
\end{figure}


In the Figure 3.7 the screen enables the users to enter their XNAT credentials. It represents a entry point to the Docker automation interface.
These credentials are used to connect securely the users to the XNAT server.
Furthermore as illustrated in the figure 3.8, after being logged in the XNAT Server the users find the second screen. this provided second 
mockup gives a clear and a straightforward representation of the automation of the Docker Container. The main advantage of this representation is that it offers a high friendliness, ensuring to all kind of users a full control of the process within the GUI.



\begin{figure}[htbp]
  \centering
  \includegraphics[width=1.1\textwidth]{mockup1.png}
  \caption{GUI Mockup 2 for Docker Container Automation in XNAT}
\end{figure}


The dashboard view, presents a comprehensive summary of all automation scripts and container activities, including key metrics such as the total number of scripts, active containers, queued jobs, and completed tasks for the day.
This approach is consistent with the current functionality of the underlying script by providing visualization for job monitoring, log access, and container lifecycle management in a manner that prioritizes usability and accessibility.
...
\begin{figure}[!ht]
  \centering
  \includegraphics[width=1.1\textwidth]{en/content/mockup2.png}
  \caption{GUI Mockup 3 for Docker Container Automation in XNAT}
\end{figure}
In the figure 3.9 illustrates the interface that appears when users select the “+ Script” button. before starting to create the image, users are prompt to specify the path to their scripts by providing a link or file location. 


\begin{figure}[!ht]
  \centering
  \includegraphics[width=1.1\textwidth]{en/content/mockup3.png}
  \caption{GUI Mockup 3 for Docker Container Automation in XNAT}
\end{figure}

The figure 3.10 illustrate the interface presented after the script path has been specified. On this screen users begin customizing the command by giving it a name and providing a brief description. Additionally, users are required to specify the associated project within XNAT.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth]{en/content/mockup4.png}
  \caption{GUI Mockup 4 for Docker Container Automation in XNAT}
\end{figure}

In the figure 3.11 displays the last screen in the workflow. In this step, a summary of the previous steps are being generated. With the aim to provide the user with a confirmation that the implementation and deployment have been completed successfully.

In summary, the presented mockup demonstrates how the developed docker automation script can be translated into a user friendly graphical interface. While the current implementation operates via a command line in the terminal, those mockups underline the potential for enhanced usability, accessibility, and reproducibility through the use of a GUI. The model based design enable the users without programming expertise to configure, initiate, and monitor containers within XNAT. However this interface has not been implemented within the scope of this thesis. The mockups serve as an intermediary layer between the technical backend and a potential future integration within the XNAT platform. Consequently, they illustrate the generalizability and long-term sustainability of the proposed approach.

\section{The Docker Container Approach and the Somnolink Project}
